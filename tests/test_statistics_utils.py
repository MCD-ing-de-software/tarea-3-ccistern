...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        expected = np.array([-1.3416407864998738, -0.4472135954999579, 0.4472135954999579, 1.3416407864998738])
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
        self.assertAlmostEqual(result.mean(), 0.0, delta=1e-7)
        self.assertAlmostEqual(result.std(), 1.0, delta=1e-7)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
        self.assertAlmostEqual(result.min(), 0.0, delta=1e-7)
        self.assertAlmostEqual(result.max(), 1.0, delta=1e-7)
